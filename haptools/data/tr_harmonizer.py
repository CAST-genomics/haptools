...
_beagle_error = (
    "If this file was imputed by Beagle, did you remember to copy the info fields over?"
)
...
def _ToVCFType(vcftype: Union[str, VcfTypes]):
    ...
    if isinstance(vcftype, str):
        if vcftype not in VcfTypes.__members__:
            raise ValueError(
                ("{} is not an excepted TR vcf type. Expected one of {}").format(
        ...
        raise TypeError(
            "{} (of type {}) is not a vcftype".format(vcftype, type(vcftype))
        )
...
def _UnexpectedTypeError(vcftype: Union[str, VcfTypes]):
    raise ValueError("self.vcftype is the unexpected type {}".format(vcftype))


def InferVCFType(
    vcffile: cyvcf2.VCF, vcftype: Union[str, VcfTypes] = "auto"
) -> VcfTypes:
    """
    ...
    if vcftype == "auto":
        if len(possible_vcf_types) == 1:
            ...
            raise TypeError(
                (
                    "Confused - this vcf looks like it could have "
    ...
    if user_supplied_type in possible_vcf_types:
        ...
        raise TypeError(
            ...
            "--vcftype {} which is not one of those types.".format(
                possible_vcf_types, vcftype
            )
        )
...
def _HarmonizeGangSTRRecord(vcfrecord: cyvcf2.Variant):
    ...
    if vcfrecord.INFO.get("RU") is None:
        raise TypeError(
            "Record at {}:{} is missing mandatory GangSTR info field RU. ".format(
                vcfrecord.CHROM, vcfrecord.POS
            )
            + _beagle_error
        )
    if vcfrecord.INFO.get("VID") is not None:
        raise TypeError(
            "Trying to read an AdVNTR record as a GangSTR record {}:{}".format(
                vcfrecord.CHROM, vcfrecord.POS
            )
        )
    if vcfrecord.INFO.get("VARID") is not None:
        raise TypeError(
            "Trying to read an EH record as a GangSTR record {}:{}".format(
                vcfrecord.CHROM, vcfrecord.POS
            )
        )
...
def _HarmonizeHipSTRRecord(vcfrecord: cyvcf2.Variant):
    ...
    """
    if (
        vcfrecord.INFO.get("START") is None
    ...
    if start_offset == 0 and neg_end_offset == 0:
        ...
        full_alleles = (vcfrecord.REF.upper(), full_alts)
    ...
    motif = InferRepeatSequence(ref_allele[start_offset:], vcfrecord.INFO["PERIOD"])
...
def _HarmonizeAdVNTRRecord(vcfrecord: cyvcf2.Variant):
    ...
    return TRRecord(
        ...
        motif,
        record_id,
        "ML" if vcfrecord.INFO.get("IMP") is None else None,
    )
...
def _HarmonizePopSTRRecord(vcfrecord: cyvcf2.Variant):
    ...
    if vcfrecord.INFO.get("Motif") is None:
        raise TypeError(
            "Record at {}:{} is missing mandatory PopSTR info field MOTIF".format(
                vcfrecord.CHROM, vcfrecord.POS
            )
        )
    ref_allele = vcfrecord.REF.upper()
    ...
    if vcfrecord.ALT:
        ...
        for alt in vcfrecord.ALT:
            ...
            if alt[0] != "<" or alt[-1] != ">":
                raise TypeError(
                    "This record does not look like a PopSTR"
    ...
    if vcfrecord.ALT:
        ...
        for alt in vcfrecord.ALT:
            ...
            if alt[:4] != "<STR" or alt[-1] != ">":
                raise TypeError(
                    "This record does not look like an EH "
                    " record. Alt alleles were not formatted"
                    " as expected"
                )
...
class TRRecord:
    ...
    def __init__(
        ...
        *,
        ...
        if full_alleles is not None and (alt_alleles is None or ref_allele is None):
            raise ValueError("Cannot set full alleles without setting regular alleles")

        if alt_allele_lengths is not None and alt_alleles is not None:
            raise ValueError(
                "Must specify only the sequences or the lengths"
                " of the alt alleles, not both."
            )

        if ref_allele_length is not None and alt_allele_lengths is None:
            raise ValueError(
                "If the ref allele is specified by length, the alt alleles must be too."
            )
        ...
        self.full_alleles_end_pos = (
            self.end_pos
            if full_alleles is None
            else round(self.full_alleles_pos + len(self.full_alleles[0]) - 1)
        )

        if alt_allele_lengths is not None:
            self.has_fabricated_alt_alleles = True
            self.alt_alleles = [
                FabricateAllele(motif, length) for length in alt_allele_lengths
            ]
        else:
        ...
        try:
            ...
            raise ValueError(
                ("Invalid TRRecord. TRRecord: {} Original record: {}").format(
                    str(self), str(self.vcfrecord)
                ),
                e,
            )
    ...
    def _CheckRecord(self):
        ...
        if len(self.alt_alleles) != len(self.vcfrecord.ALT):
            raise ValueError(
                "Underlying record does not have the same "
        ...
        if self.full_alleles:
            if len(self.full_alleles) != 2:
                raise ValueError(
                    "full_alleles doesn't have both a ref allele and alt alleles"
                )
            full_ref, full_alts = self.full_alleles
            if len(full_alts) != len(self.alt_alleles):
                raise ValueError(
                    "Different number of full alternate alleles than normal alt alleles"
                )
            if self.ref_allele not in full_ref:
                raise ValueError("could not find ref allele inside full ref allele")
            for idx, (full_alt, alt) in enumerate(zip(full_alts, self.alt_alleles)):
                if alt not in full_alt:
                    raise ValueError(
                        (
                            "Could not find alt allele {} inside its full alt allele"
                        ).format(idx)
                    )
    ...
    def GetCalledSamples(self, strict: bool = True) -> Optional[np.ndarray]:
        ...
        if strict:
            ...
            return ~np.all(
                np.logical_or(gt_idxs[:, :-1] == -1, gt_idxs[:, :-1] == -2), axis=1
            )
    ...
    def GetSamplePloidies(self) -> Optional[np.ndarray]:
        ...
        return gt_idxs.shape[1] - 1 - np.sum(gt_idxs[:, :-1] == -2, axis=1)
    ...
    def _GetStringGenotypeArray(self, idx_gts: np.ndarray, seq_alleles: List[str]):
    ...
    def GetStringGenotypes(self) -> Optional[np.ndarray]:
        ...
        if self.HasFabricatedAltAlleles():
            warnings.warn(
                "String genotypes have been requested for a"
    ...
    def GetGenotypeCounts(
        ...
        if index and not uselength:
            raise ValueError(
                "Specified uselength=False and index at the same"
        ...
        genotypes, counts = np.unique(gts, axis=0, return_counts=True)
    ...
    def GetAlleleCounts(
        ...
        *,
        ...
        alleles, counts = np.unique(gts, return_counts=True)
    ...
    def GetMaxAllele(self, sample_index: Optional[Any] = None) -> float:
        """
        ...
        alleles = self.GetAlleleCounts(uselength=True, sample_index=sample_index).keys()
        if len(alleles) == 0:
            return np.nan
        return max(alleles)
    ...
    def HasQualityScores(self) -> bool:
        ...
        """
        return self.quality_field is not None and self.quality_field in self.format
    ...
    def GetQualityScores(self) -> np.ndarray:
        ...
        if not self.HasQualityScores():
            raise TypeError(
                "This TRRecord does not have a corresponding quality score field"
            )
    ...
    def __str__(self):
        ...
        if record_id is None:
            record_id = "{}:{}".format(self.vcfrecord.CHROM, self.vcfrecord.POS)
        if self.HasFullStringGenotypes():
            string = "{} {} {} ".format(record_id, self.motif, self.full_alleles[0])
            string += ",".join(self.full_alleles[1])
            return string

        if self.HasFabricatedRefAllele():
            string = "{} {} n_reps:{} ".format(
                record_id, self.motif, self.ref_allele_length
            )
        else:
        ...
        if len(self.alt_alleles) == 0:
            ...
            string += ",".join(
                "n_reps:" + str(length) for length in self.alt_allele_lengths
            )
        else:

...
from haptools.data import GenotypesVCF, GenotypesPLINK, GenotypesTR, GenotypesPLINKTR
...
def create_variant_files(gts, intervals, num_samps):
    ...
    for val in intervals:
        ...
        # write VCF files
        vcf = GenotypesTR(sub.fname.with_suffix(".vcf"))
        vcf.data = sub.data
...
def write_TR_VCF(gts):
    ...
    vcf.header.add_meta(
        ...
        items=[
            ...
            ("Number", 1),
            ("Type", "Integer"),
            ("Description", "Inclusive start coodinate for the repetitive portion of the reference allele"),
        ],
    )
    vcf.header.add_meta(
        "INFO",
        items=[
            ...
            ("Number", 1),
            ("Type", "Integer"),
            ("Description", "Inclusive end coordinate for the repetitive portion of the reference allele"),
        ],
    )
    vcf.header.add_meta(
        "INFO",
    ...
    try:
        vcf.header.add_samples(gts.samples)
        ...
        for sample in gts.samples:
            vcf.header.add_sample(sample)
    gts.log.info("Writing VCF records")
    phased = gts._prephased or (gts.data.shape[2] < 3)
